...
let isSwiping = false
let initSpeedDownAndUp = true

export const detectSwipe = (scrollDelta, deltaTime, onSwipe, onSwipeEnd) => {
...
export default class Xapp {
  ...
  constructor() {
    ...
    document.addEventListener("dblclick", this.onTap.bind(this))
...
export class AssetLoader {
  ...
  constructor(assets) {
    ...
    loadingManager.onError = e => {throw new Error(e)}
  }
  ...
  __onProgress(url, number, total) {
    const mal = this.mustAseetsLen
    const al = this.allAssetsLen
    const progressMal = number / mal
    const progressAll = (number - mal) / al

    if (mal >= number && progressMal <= 1){
      this.emitter.emit('mustAssetsLoading',{progress:progressMal})
    }

    if (mal < number && al >= (number - mal) && progressAll <= 1){
      this.emitter.emit('allAssetsLoading',{progress:progressAll})
    }
  }
} 