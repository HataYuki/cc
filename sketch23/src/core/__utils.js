...
class ScrollManager {
  ...
  setup() {
    this.__element.addEventListener(
      'wheel',
      this.__onWheel,
      ScrollManager.LISTENER_OPTIONS
    )
    this.__element.addEventListener(
      'touchstart',
      this.__onTouchStart,
  ...
  __onScroll(deltaX, deltaY, event) {
    const newDeltaX = clamp(
      deltaX,
      -1 * this.options.deltaXMax,
      this.options.deltaXMax
    )
    const newDeltaY = clamp(
      deltaY,
      -1 * this.options.deltaYMax,
      this.options.deltaYMax
    )
    ...
    const needUpdate =
      this.__delta.x !== newDeltaX || this.__delta.y !== newDeltaY
    ...
    if (needUpdate) {
      ...
      this.__emitter.emit('scroll', {
        delta: this.__delta.clone(),
        event: event,
      })
    }
  ...
  __simulateInertia() {
    const newDelta = this.__delta
      .clone()
      .multiplyScalar(ScrollManager.INERTIA_DECAY)

    if (Math.abs(newDelta.x) <= 0.1) newDelta.x = 0
    if (Math.abs(newDelta.y) <= 0.1) newDelta.y = 0

    this.__rafScrollId = requestAnimationFrame(() =>
      this.__onScroll(newDelta.x, newDelta.y, {})
    )
  }

  __needsInertia(event) {
    return (
      event.type === 'touchend' || this.__delta.x !== 0 || this.__delta.y !== 0
    )
  }
    ...
    this.__startInertiaTimer = setTimeout(() => {
      if (this.__needsInertia(event)) {
        const delta = this.__delta
          ...
          .multiplyScalar(ScrollManager.INERTIA_DECAY)
        this.__rafScrollId = requestAnimationFrame(() =>
          this.__onScroll(delta.x, delta.y, {})
        )
      }
  ...
  __onTouchMove = event => {
    ...
    this.__rafTouchMoveId = requestAnimationFrame(() =>
      this.__touchMoveUpdate(event)
    )
  }
  ...
  __touchMoveUpdate = event => {
    ...
    const deltaX =
      -(clientX - this.__touchStart.x) * this.options.touchMultiplier
    const deltaY =
      -(clientY - this.__touchStart.y) * this.options.touchMultiplier
...
class SwipeManager {
  ...
  update(scrollDeltaX, scrollDeltaY, deltaTime) {
    ...
    if (speedX) {
      ...
      if (isUp && speedX > 0.5 && !this.__xSwiped) {
        ;(this.__emitter.emit('startSwipeX', { dir }),
          (this.__xSwiped = true),
          (this.__needsUpdateX = true))
      }
      if (isDown && this.__xSwiped) {
        ;(this.__emitter.emit('endSwipeX', { dir }),
          (this.__xSwiped = false),
          (this.__needsUpdateX = true))
      }
    }
    ...
    if (speedY) {
      ...
      if (isUp && speedY > 0.5 && !this.__ySwiped) {
        ;(this.__emitter.emit('startSwipeY', { dir }),
          (this.__ySwiped = true),
          (this.__needsUpdateY = true))
      }
      if (isDown && this.__ySwiped) {
        ;(this.__emitter.emit('endSwipeY', { dir }),
          (this.__ySwiped = false),
          (this.__needsUpdateY = true))
      }
    }
  }
  ...
  __pushDelta(deltaX, deltaY) {
    if (deltaX)
      this.__push('__delta_x_buffer', deltaX, SwipeManager.DELTA_BUFFER_SIZE)
    if (deltaY)
      this.__push('__delta_y_buffer', deltaY, SwipeManager.DELTA_BUFFER_SIZE)
  }

  __pushHistory(smoothX, smoothY) {
    if (smoothX)
      this.__push('__smooth_x_his', smoothX, SwipeManager.DELTA_HISTORY_SIZE)
    if (smoothY)
      this.__push('__smooth_y_his', smoothY, SwipeManager.DELTA_HISTORY_SIZE)
  }
...
class AssetManager {
  ...
  constructor(assets) {
    ...
    this.__loadingManager.onProgress = (url, load, total) =>
      this.__onProgress.apply(this, [url, load, total])
  ...
  setup(assets) {
    ...
    this.__mustAssetsLen = Object.keys(this.__assets).filter(
      k => this.__assets[k].must
    ).length
    this.__nonMustAssetsLen = Object.keys(this.__assets).filter(
      k => !this.__assets[k].must
    ).length
    return this
  }
  ...
  async whenMustLoad() {
    if (!this.__mustAssetsLen)
      return Promise.resolve().then(() => (this.__mustLoaded = true))

    return Promise.all(
      Object.keys(this.__assets)
        .filter(key => this.__assets[key].must)
        .map(key => this.__load(key))
    ).then(
      () => (this.__mustLoaded = true),
      (this.__needsUpdateLoadedMust = true)
    )
  }
  ...
  async load() {
    ...
    const checkNotMust = key =>
      this.__assets[key].hasOwnProperty('must') && !this.__assets[key].must
    ...
    return Promise.all(
      ...
      (this.__needsUpdateLoadedall = true)
    )
  }
  ...
  __load(key) {
    ...
    return new Promise((resolve, reject) => {
      this.__selectLoader(asset.type).load(asset.url, data => {
  ...
  __onProgress(url, number) {
    if (!this.__mustLoaded) {
      ...
      this.__emitter.emit('mustAssetsLoading', {
        progress: this.__mustProgress,
      })
    } else {
      this.__nonMustProgress =
        (number - this.__mustAssetsLen) / this.__allAssetsLen
      this.__emitter.emit('nonMustAssetsLoading', {
        progress: this.__nonMustProgress,
      })
    }
  }
...
class CursorManager {
  ...
  dispose() {
    ...
    this.__emitter.clearListeners()
    if (this.__rafId)
      (cancelAnimationFrame(this.__rafId), (this.__rafId = null))
  }
}
...
class Xapp {
  ...
  constructor() {
    this.inputKeyManager
      .on('keyPressed', arg =>
        this.onKeyPressed(arg.event, arg.isCombKey, arg.isAnyKey)
      )
      .on('keyReleased', arg => this.onKeyReleased(arg.event))

    this.scrollManager.on('scroll', arg => {})
...
export default class Xdraw extends Xapp {
  ...
  async __setup() {
    ...
    this.assetsManager
      ...
      .on('mustAssetsLoading', p => {
        this.mustAssetsProgress = p.progress
      })
      .on(
        'nonMustAssetsLoading',
        p => (this.nonMustAssetsProgress = p.progress)
      )
    ...
    if (!this.canvas)
      throw new Error(`Canvas element with ID '${this.CANVAS_ID}' not found`)
    ...
    this.renderer.setPixelRatio(
      Math.min(window.devicePixelRatio, this.PIXEL_RATIO)
    )
    ...
    // Custom post process
    this.effect(effect => this.effectComposer.addPass(effect), ShaderPass)
  ...
  update(time, deltaTime) {
    ...
    this.assetsManager.consumeStartedLoadingAll(
      this,
      Xdraw.START_LOAD_ALL_ASSETS_FRAG
    )
  ...
  onSwipeXStart(dir) {
    this.swipe(new THREE.Vector3(1, 0, dir))
  }
  onSwipeXEnd(dir) {
    this.swipe(new THREE.Vector3(1, 0, dir))
  }
  onSwipeYStart(dir) {
    this.swipe(new THREE.Vector3(0, 1, dir))
  }
  onSwipeYEnd(dir) {
    this.swipe(new THREE.Vector3(0, 1, dir))
  }
