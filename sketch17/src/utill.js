...
class AssetManager {
  ...
  constructor(assets) {
    ...
    this.loadingManager.onProgress = (url, load, total) => this.__onProgress.apply(this,[url, load, total])
    this.loadingManager.onError = (url) => console.error(`Failed to load asset: ${url}`)
  }
  ...
  async load() {
    this.needsUpdateStartLoadAll = true
  ...
  consumeStartLoadAll(object, keyName) {
    const wasNeeded = this.needsUpdateStartLoadAll
    this.needsUpdateStartLoadAll = false
    if (object && keyName) object[keyName] = wasNeeded
    return wasNeeded
  }
...
export default class Xdraw extends Xapp{
  ...
  update(time, deltaTime) {
    ...
    this.assetsManager.consumeStartLoadAll(this, Xdraw.START_LOAD_ALL_ASSETS_FRAG)
    this.assetsManager.consumeLoadedAll(this, Xdraw.ALL_ASSETS_LOADED_FRAG)

    if (this[Xdraw.START_LOAD_ALL_ASSETS_FRAG]) this.__isContinuUpdate = true
      
    if (!this.__isContinuUpdate) return
